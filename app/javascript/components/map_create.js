...
export const mapCreate = () => {
  ...
  if (root) {
    ...
    // locationPointsCreate()   地点情報をもとに各座標地点に対象コンテナの画像を配置する
    // locationViewCreate()     locationPointsをクリックした際にクリック対象に対応した画像を表示する
    if (image !== null) {
      useEffect(() => {
      ...
      return h`
        ...
        >
          ...
          ${locationPointsCreate(containers, locations, zoomLevel, srcCoord, setViewData, showFlag, setShowFlag)}
          ${locationViewCreate(loggedIn, viewData, showFlag, setShowFlag)}
        </div>
      `
    } else {
      return h`<p class="m-3">対象の地図画像が登録されていません。</p>`
    }
  }
}
